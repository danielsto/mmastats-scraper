...
def get_player_data(player, row, season, mode):
    ...
    if status_code == 200:
        ...
        team_id = (
            soup.find("tr", {"id": f"per_game.{get_end_year(season)}"})
            .find("td", {"data-stat": "team_id"})
            .text
            if soup.find("tr", {"id": f"per_game.{get_end_year(season)}"})
            else None
        )
        team = TEAMS[team_id] if team_id else None

        birth_date = (
            soup.find("span", {"itemprop": "birthDate"})["data-birth"]
            if soup.find("span", {"itemprop": "birthDate"})
            else None
        )
        age_awarded = (
            get_end_year(season) - int(birth_date.split("-")[0]) if birth_date else None
        )
